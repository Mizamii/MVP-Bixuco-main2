...
// src/adapters/yup.ts
function yupValidator(yupInstance) {
  yupInstance.addMethod(yupInstance.string, "cpf", function cpfMethod(message = "CPF inv\xE1lido") {
    return this.test("cpf", message, (value) => !value || cpf_default.isValid(value));
  });
  yupInstance.addMethod(yupInstance.string, "cnpj", function cnpjMethod(message = "CNPJ inv\xE1lido") {
    return this.test("cnpj", message, (value) => !value || cnpj_default.isValid(value));
  });
}
